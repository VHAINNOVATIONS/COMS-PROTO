...
//        }else {
  //          $this->set('patients',null);
    ...
    function adminDaysArray($therapies, $infusionMap, $dateStarted, $template, $cycle, $patientId){
...
//                    echo '<br/>';
                    ...
                    if(null != $infusionMap){
                        ...
                        $this->insertHydrations($therapy, $infusionMap, $templateid[0]['lookupid']);
					
                    }else{
                        //$this->insertRegimens($therapy, $templateid[0]['lookupid'], $Order_IDR);
                        $this->insertRegimens($therapy, $templateid[0]['lookupid']);
...
//                echo '*************';
    ...
    function insertRegimens($regimen,$templateid){
        ...
        $retVal = $lookup->saveRegimen($regimens, $templateid);
	...
	function insertHydrations($hydration,$infusionMap,$templateid){
        ...
        $lookup = new LookUp();
        $retVal = $lookup->saveHydrations($hydrations, $hydration["type"], $templateid);
    ...
    function createOEMRecords($form_data){
        ...
        for ($index = 1; $index <= $totalCourses; $index++) {
            ...
            if(!is_null($postHydrations)){
                ...
                $this->adminDaysArray($postHydrations, $postinfusionMap, $dateStarted, $template,$index,$patientId);
            ...
            if(!is_null($regimens)){
                ...
                $this->adminDaysArray($regimens, null, $dateStarted, $template,$index,$patientId);
